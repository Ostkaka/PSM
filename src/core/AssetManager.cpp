...
namespace psm
{
	...
	IAssetHandler& AssetManager::getHandler(const typeAssetHandlerID assetHandlerID) const
	{
		...
		// Make sure we aren't returning nullptr at this point
		if(handler == nullptr)
		{
			;			
		}
	...
	void AssetManager::registerHandler(IAssetHandler* assetHandler)
	{
		...
		// Make sure the Handler provided is not nullptr
		if(nullptr != assetHandler)
		{
			// Try to find the asset using theAssetID as the key
		...
		{
			// Log an error for trying to register a nullptr handler
			//FLOG(StatusError)
		//		<< "AssetManager:RegisterHandler() Handler pointer provided was nullptr!"
			//	<< std::endl;
			;
		}
	}
